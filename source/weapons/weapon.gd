...
@onready var laser := $MuzzlePivot/Laser
...
func adjust_muzzle(c_point : Vector3, _c_normal : Vector3):
	...
	laser.mesh.height = muzzle_pivot.global_position.distance_to(c_point)
	laser.position.z = -muzzle_pivot.global_position.distance_to(c_point) * 0.5
